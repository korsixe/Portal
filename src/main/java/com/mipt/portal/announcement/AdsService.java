...
//                System.out.println(ad.toString());
    ...
    public List<Announcement> getUserAds(Long userId) {
        ArrayList<Long> idList = userService.findUserById(userId).getData().getAdList();
        ArrayList<Announcement> adList = new ArrayList<>();
        for (int i = 0; i < idList.size(); i++) {
            Announcement ad = getAd(idList.get(i));
            if (ad != null) {
                adList.add(ad);
            }
        }
