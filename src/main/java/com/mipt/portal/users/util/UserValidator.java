...
public class UserValidator {
    private static final List<String> STRENGTH_CRITERIA = List.of("!", "?", "@", "#", "$", "%", "&", "*", "_", "-");
    ...
    public static boolean validateEmail(String email) {
        ...
        if (!email.matches(emailPattern)) {
            throw new IllegalArgumentException("Неправильный формат почты. Пример физтех-почты: ivanov.ii@phystech.edu");
        }
    ...
    public static boolean isPasswordStrong(String password) {
        ...
        for (char i : password.toCharArray()) {
            if (Character.isLowerCase(i)) hasLower = 1;
            if (Character.isUpperCase(i)) hasUpper = 2;
            if (Character.isDigit(i)) hasDigit = 1.5;
            if (STRENGTH_CRITERIA.contains(String.valueOf(i))) hasSpecialChar = 4;
        }

        if (password.length() > 10) goodSize = 1.5;
